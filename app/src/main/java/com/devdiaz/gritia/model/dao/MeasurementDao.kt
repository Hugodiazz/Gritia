...
interface MeasurementDao {
        ...
        suspend fun getUserMetricByDateRange(
                userId: Long,
                start: Long,
                end: Long
        ): UserMetricEntity?
        ...
        @Query(
                "SELECT * FROM user_metrics WHERE user_id = :userId AND recorded_at < :timestamp ORDER BY recorded_at DESC LIMIT 1"
        )
        suspend fun getLatestUserMetricBeforeDate(userId: Long, timestamp: Long): UserMetricEntity?
        ...
        @Query(
                "SELECT * FROM body_measurements WHERE user_id = :userId AND recorded_at < :timestamp ORDER BY recorded_at DESC LIMIT 1"
        )
        suspend fun getLatestBodyMeasurementBeforeDate(
                userId: Long,
                timestamp: Long
        ): BodyMeasurementEntity?
